...
class Classify:
    def __init__(self):
        ...
        self.logger.debug("Start classify")
    ...
    def classification(self, channel, method, props, body):
        ...
        if r.get('centers') is None:
            centers = self.get_centers_from_db()
            self.logger.debug("Centers from db")
        else:
        ...
        for name in centers:
            distance = self.face_distance(centers[name][0], embedding)
        ...
        if face_distances[min_dist_cluster] <= centers[min_dist_cluster][1]:
            ...
            dist_type = "cluster"
            self.save_emb_to_db(found_cluster, embedding)
        ...
        self.logger.debug("redis {}".format(r.get(str(session_id))))
        if r.get(str(session_id)) is not None:
            return
        ...
        if found_cluster != 'Unknown':
            found_cluster = self.get_username(int(found_cluster))
        ...
        self.logger.debug("Photo push: {}".format(t1 - t0))
        self.logger.debug("Photo push -> detector: {}".format(t2 - t1))
        self.logger.debug("detector: {}".format(t3 - t2))
        self.logger.debug("detector -> classify: {}".format(t6 - t3))
        self.logger.debug("classify: {}".format(t7 - t6))
        self.logger.debug("All time: {}".format(t7 - t0))
...
if __name__ == '__main__':
    ...
    queue = RabbitConnection(args.config, classify.logger)
    # queue.create_exchange(config['rabbitmq']['exchange'])
    queue.create_queue('classify')
    queue.read_queue(classify.classification)
